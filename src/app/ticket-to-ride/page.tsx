...
export default function TtrLanding() {
  ...
  function handleCreate() {
    ...
    router.push(`/ticket-to-ride/oda/${room}`);
  }
      ...
      <div className="w-full max-w-md space-y-6 rounded-3xl border border-white/10 bg-slate-900/60 p-8 shadow-2xl backdrop-blur">
        <header className="space-y-1 text-center">
          ...
          <p className="text-sm text-white/60">
            ABD haritası üzerinde tren yolu inşa et. 2–5 oyuncu, en uzun rota
            +10.
          </p>
        </header>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  setError(null);
                  if (!commitNickname()) return;
                  router.push(`/ticket-to-ride/oda/${lastRoom}`);
                }}
        ...
        <p className="text-center text-[11px] text-white/30">
          Bu oyun bir kişisel proje denemesidir. Ticket to Ride® markasıyla
          bağlantılı değildir.
        </p>
      </div>
    </main>
